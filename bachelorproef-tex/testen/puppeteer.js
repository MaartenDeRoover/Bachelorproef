...
describe('Puppeteer ', async () => {
  ...
  before('Going to the site and logging in', async () => {
    ...
    await page.goto('http://puppeteer.local', { timeout: 0, waitUntil: 'domcontentloaded' });
    title = await page.evaluate(() => { return document.querySelector('#block-bartik-page-title').textContent; });
  ...
  it('going to the site and asserting the title is correct', async () => {
    ...
    title = await page.evaluate(() => { return document.querySelector('#block-seven-page-title').textContent; });
